...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "product insight"
      decision: "Simple status is enough"
    - topic: "primary persona scope"
      decision: "Individual homeowner"
    - topic: "auth strategy"
      decision: "Login"
    - topic: "role model"
      decision: "Flat user model"
    - topic: "mvp timeline"
      decision: "One week of after-hours work"
    - topic: "secondary success"
      decision: "Fast status scan"
    - topic: "mvp guardrail"
      decision: "Private task data"
    - topic: "due soon threshold"
      decision: "Within 7 days"
    - topic: "response timing"
      decision: "Under 3 seconds"
    - topic: "privacy commitment"
      decision: "Own tasks only"
    - topic: "browser/device support"
      decision: "Modern desktop/mobile browsers"
    - topic: "product type"
      decision: "Website or web app"
    - topic: "target scale"
      decision: "Just me or a few users"
    - topic: "scale effect on domain rule"
      decision: "No change; due-date and status rule is based only on each task's last completion date and recurrence interval"
    - topic: "hard deadline"
      decision: "2026-09-14"
    - topic: "work timing"
      decision: "After-hours work"
    - topic: "non-goals"
      decision: "No shared homes, no reminders, no task library, and no AI-generated maintenance schedules or recommendations"
  frs_drafted: 6
...
I want to build a simple web application called HomeKeep for homeowners to manage recurring home maintenance tasks. Users create maintenance tasks by providing a name, the date the task was last completed, and how often it should be repeated. The application calculates the next due date and tells the user whether the task is OK, due soon, or overdue. I want the MVP to be as small as reasonably possible.
...
The due-date and status rule remains per-task at larger scale: it is based only on each task's last completion date and recurrence interval.
...
- Timeline-cost acknowledgment: present; MVP target is 1 week.
- Non-Goals: present.
